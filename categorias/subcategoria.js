...
function rootBackground() {
    const imagenes = {
        grande: {
            dia: '../imagenes/boulevard/Boul_mañana.png',
            tarde: '../imagenes/boulevard/Boul_tarde.png',
            noche: '../imagenes/boulevard/Boul_noche.png'
        },
        mediana: {
            dia: '../imagenes/boulevard/Boul_mañana.png',
            tarde: '../imagenes/boulevard/Boul_tarde.png',
        ...
        pequena: {
            dia: '../imagenes/boulevard/Boul_mañana-Celular.png',
            tarde: '../imagenes/boulevard/Boul_tarde-Celular.png',
            noche: '../imagenes/boulevard/Boul_noche-Celular.png'
        }
    };

    const mediaGrande = window.matchMedia('(min-width: 1025px)');
    const mediaMediana = window.matchMedia('(min-width: 721px) and (max-width: 1024px)');
    ...
    const period = hour >= 7 && hour < 13 ? 'dia' : (hour >= 13 && hour < 19 ? 'tarde' : 'noche');
    const type = mediaGrande.matches ? 'grande' : (mediaMediana.matches ? 'mediana' : 'pequena');

    return imagenes[type][period];
}
...
function cardHtml(business) {
    ...
    const icon = esc(currentSub.icon || '🏪');
    const logoHtml = logo
        ? `<img class="ux-company-logo-img" src="${esc(logo)}"
                alt="Logo de ${esc(business.name || 'negocio')}"
                loading="lazy">`
        : `<div class="ux-company-logo-placeholder">${icon}</div>`;
    const featured = isFeatured(business)
        ? '<span class="ux-company-featured">Destacado</span>'
        : '';
    const status = statusText(business);
    const address = business.address
        ? esc(business.address)
        : esc(business.city || 'Loja, Ecuador');
    const description = esc(
        business.description ||
        business.short_description ||
        `Negocio especializado en ${currentSub.name}.`
    );
    const city = esc(business.city || 'Loja');

    return `
        <a class="ux-card" href="${esc(businessLink(business))}"
            aria-label="Ver negocio ${esc(business.name || 'Urbanex')}">
            <div class="ux-company-head">
                <div class="ux-company-logo">${logoHtml}</div>
                <div class="ux-company-title">
                    <div class="ux-company-category">${icon} ${esc(currentSub.name)}</div>
                    <h3>${esc(business.name || 'Negocio')}</h3>
                    ${featured}
                </div>
            </div>
            <div class="ux-card-body">
                <p class="ux-company-description">${description}</p>
                <div class="ux-company-meta">
                    <div class="ux-company-location">
                        <span class="ux-meta-icon" aria-hidden="true">⌖</span>
                        <span><small>Ubicación</small>${address}</span>
                    </div>
                    <div class="ux-company-city">
                        <span class="ux-meta-icon" aria-hidden="true">⌂</span>
                        <span><small>Ciudad</small>${city}</span>
                    </div>
                </div>
                <div class="ux-card-bottom">
                    <span class="ux-badge"><i></i>${esc(status)}</span>
                    <span class="ux-card-cta">Conocer empresa <b>→</b></span>
                </div>
            </div>
        </a>
    `;
...
async function boot() {
    ...
    if (categoriesResult.error) {
        ...
        showLoadError('No se pudieron cargar las categorías.');
        return;
    }

    if (businessesResult.error) {
        console.error(businessesResult.error);
        showLoadError('No se pudieron cargar los negocios.');
        return;
    }
    ...
    allBusinesses = (businessesResult.data || [])
        ...
        .filter(business =>
            !window.UrbanexLocation || window.UrbanexLocation.matchesBusiness(business)
        );
...
const urbanexSubcategoryMediaGrande = window.matchMedia('(min-width: 1025px)');
const urbanexSubcategoryMediaMediana = window.matchMedia('(min-width: 721px) and (max-width: 1024px)');
const urbanexSubcategoryMediaPequena = window.matchMedia('(max-width: 720px)');

function updateUrbanexSubcategoryBackground() {
    document.documentElement.style.setProperty('--page-bg', `url("${rootBackground()}")`);
}

setInterval(updateUrbanexSubcategoryBackground, 60000);
[
    urbanexSubcategoryMediaGrande,
    urbanexSubcategoryMediaMediana,
    urbanexSubcategoryMediaPequena
].forEach(media => {
    if (media.addEventListener) media.addEventListener('change', updateUrbanexSubcategoryBackground);
    else if (media.addListener) media.addListener(updateUrbanexSubcategoryBackground);
});
